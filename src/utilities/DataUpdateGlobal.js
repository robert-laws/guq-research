...
export const DataUpdateGlobal = () => {
  ...
  const dataCollectionAuthorsUpdate = async () => {
  ...
  const handleClickAuthorsUpdate = () => {
    setLoading(true);

    dataCollectionAuthorsUpdate();
    ...
    <div>
      ...
      <div className='flex items-center justify-center'>
        ...
        <button
          type='button'
          className={`${
            loadingThree ? 'cursor-not-allowed' : 'cursor-pointer'
          } mr-4 inline-flex items-center px-4 py-2 font-semibold leading-6 text-sm shadow rounded-md text-white bg-indigo-800 hover:bg-indigo-700 transition ease-in-out duration-150`}
          disabled={loadingThree}
          onClick={handleClickAuthorsUpdate}
        >
          <svg
            className={`${
              loadingThree ? 'animate-spin -ml-1 mr-3 h-5 w-5 text-white' : ''
            }`}
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
            viewBox='0 0 24 24'
